...
def prometheus_port_forward(kubeconf: str) -> None:
    os.execvp(
        "kubectl",
        [
            "kubectl",
            ...
            kubeconf,
            "port-forward",
            "svc/aptos-node-mon-aptos-monitoring-prometheus",
            "9090"
        ]
    )
...
class LocalForgeRunner(ForgeRunner):
    def run(self, context: ForgeContext) -> ForgeResult:
        ...
        port_forward_process = context.processes.spawn(lambda: prometheus_port_forward(context.forge_cluster.kubeconf))
