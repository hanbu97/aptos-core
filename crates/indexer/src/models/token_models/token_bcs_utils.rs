...
pub fn convert_bcs_propertymap(s: Value) -> Option<String> {
    ...
    D: Deserializer<'de>,
{
    ...
    Ok(convert_bcs_propertymap(s.clone()).unwrap_or_else(|| s.to_string()))
}
...
fn test_deserialize_propertymap_from_bcs() {
    ...
    assert_eq!(b["data"]["Sergi"]["value"], "10");
    assert_eq!(b["data"]["Yuri"]["value"], "hello");
}